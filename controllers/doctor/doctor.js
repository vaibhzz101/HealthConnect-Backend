...
const getTokenDoctor = async(req, res) => {
    ...
    try{
        ...
        if(!olddoctor){
            ...
            if(isPassCorrect){
                const Token = await jwt.sign({name:olddoctor.name,role:olddoctor.role,email:olddoctor.email},"djdhjzbyw8ry89y",{ expiresIn: "1d" });
                const refreshToken = await jwt.sign({name:olddoctor.name,role:olddoctor.role,email:olddoctor.email},"w8yr893ur8943yr8y389",{ expiresIn: "7d" });
